...
if [ "null" == "true" ]; then
    mkdir results
    gzip -cd empty.fna.gz > results/test-ont.fna
elif [[ "ont" == "hybrid" || "false" == "true" ]]; then
    # Unicycler
    unicycler \
        -1 test-ont.fastq.gz -2 null \
        --keep 1 --min_fasta_length 500 --mode normal --min_component_size 1000 --min_dead_end_size 1000 \
         \
        -o results/ \
        --threads 4
    sed -r 's/^>([0-9]+)(.*)/>test-ont_\1\2/' results/assembly.fasta > results/test-ont.fna
    mv results/assembly.gfa results/unicycler.gfa
elif [[ "ont" == "ont" || "ont" == "short_polish" ]]; then
    # Dragonflye
    if ! dragonflye \
        --reads test-ont.fastq.gz \
        --gsize 358000 \
        --outdir results \
        --assembler flye --polypolish 1 --minlen 500 --mincov 2 --force --keepfiles --depth 0 --minreadlen 0 --minquality 0 --racon 1 --medaka 0 \
        --namefmt "test-ont_%05d" \
        --cpus 4 \
        --ram 7; then
    ...
    mv results/contigs.fa results/test-ont.fna
else
    # Shovill
    if ! shovill-se --SE test-ont.fastq.gz \
        --gsize 358000 \
        --outdir results \
...
TOTAL_CONTIGS=`grep -c "^>" results/test-ont.fna || true`
if [ "${TOTAL_CONTIGS}" -gt 0 ]; then
    assembly-scan results/test-ont.fna --prefix test-ont > results/test-ont.tsv
    TOTAL_CONTIG_SIZE=$(cut -f 3 results/test-ont.tsv | tail -n 1)
    if [ "${TOTAL_CONTIG_SIZE}" -lt 100000 ]; then
        mv results/test-ont.fna results/test-ont-error.fna
        mv results/test-ont.tsv results/test-ont-error.tsv
        echo "test-ont assembled size (${TOTAL_CONTIG_SIZE} bp) is less than the minimum allowed genome
                size (100000 bp). If this is unexpected, please investigate test-ont to
                ...
                based analysis of test-ont will be discontinued." |             sed 's/^\s*//' > test-ont-assembly-error.txt
    fi
else
    mv results/test-ont.fna results/test-ont-error.fna
    echo "test-ont assembled successfully, but 0 contigs were formed. Please investigate
            test-ont to determine a cause (e.g. metagenomic, contaminants, etc...) for this
            outcome. Further assembly-based analysis of test-ont will be discontinued." |         sed 's/^\s*//' > test-ont-assembly-error.txt
fi
