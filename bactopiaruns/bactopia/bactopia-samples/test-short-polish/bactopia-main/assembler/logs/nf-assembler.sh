...
if [ "null" == "true" ]; then
    mkdir results
    gzip -cd test-short-polish.fastq.gz > results/test-short-polish.fna
elif [[ "short_polish" == "hybrid" || "false" == "true" ]]; then
    # Unicycler
    unicycler \
        -1 test-short-polish_R1.fastq.gz -2 test-short-polish_R2.fastq.gz \
        --keep 1 --min_fasta_length 500 --mode normal --min_component_size 1000 --min_dead_end_size 1000 \
         \
        -o results/ \
        --threads 4
    sed -r 's/^>([0-9]+)(.*)/>test-short-polish_\1\2/' results/assembly.fasta > results/test-short-polish.fna
    mv results/assembly.gfa results/unicycler.gfa
elif [[ "short_polish" == "ont" || "short_polish" == "short_polish" ]]; then
    # Dragonflye
    if ! dragonflye \
        --reads test-short-polish.fastq.gz --R1 test-short-polish_R1.fastq.gz --R2 test-short-polish_R2.fastq.gz \
        --gsize 358000 \
        --outdir results \
        --assembler flye --polypolish 1 --minlen 500 --mincov 2 --force --keepfiles --depth 0 --minreadlen 0 --minquality 0 --racon 1 --medaka 0 \
        --namefmt "test-short-polish_%05d" \
        --cpus 4 \
        --ram 7; then
    ...
    mv results/contigs.fa results/test-short-polish.fna
else
    # Shovill
    if ! shovill --R1 test-short-polish_R1.fastq.gz --R2 test-short-polish_R2.fastq.gz \
        --gsize 358000 \
        --outdir results \
...
TOTAL_CONTIGS=`grep -c "^>" results/test-short-polish.fna || true`
if [ "${TOTAL_CONTIGS}" -gt 0 ]; then
    assembly-scan results/test-short-polish.fna --prefix test-short-polish > results/test-short-polish.tsv
    TOTAL_CONTIG_SIZE=$(cut -f 3 results/test-short-polish.tsv | tail -n 1)
    if [ "${TOTAL_CONTIG_SIZE}" -lt 100000 ]; then
        mv results/test-short-polish.fna results/test-short-polish-error.fna
        mv results/test-short-polish.tsv results/test-short-polish-error.tsv
        echo "test-short-polish assembled size (${TOTAL_CONTIG_SIZE} bp) is less than the minimum allowed genome
                size (100000 bp). If this is unexpected, please investigate test-short-polish to
                ...
                based analysis of test-short-polish will be discontinued." |             sed 's/^\s*//' > test-short-polish-assembly-error.txt
    fi
else
    mv results/test-short-polish.fna results/test-short-polish-error.fna
    echo "test-short-polish assembled successfully, but 0 contigs were formed. Please investigate
            test-short-polish to determine a cause (e.g. metagenomic, contaminants, etc...) for this
            outcome. Further assembly-based analysis of test-short-polish will be discontinued." |         sed 's/^\s*//' > test-short-polish-assembly-error.txt
fi
